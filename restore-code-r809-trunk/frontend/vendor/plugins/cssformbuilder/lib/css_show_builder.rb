...
class CssShowBuilder <  ActionView::Helpers::ShowBuilder
  ...
  def text_area(field, options = {})
    ...
    process(field,options,'<pre>' + text+ '</pre>')
  end
  ...
  def date_field(field, html_options = {}, date_options={})
    ...
    process(field,html_options,value)
  end
  
  def fields_for(object_name, *args, &proc)
    options = args.last.is_a?(Hash) ? args.pop : {}
    options[:template]= @template
    options[:builder]= CssShowBuilder
...
def show_fields_for(object_name, *args, &proc)
  ...
  yield((options[:builder] || ActionView::Helpers::ShowBuilder).new(object_name, object, template, options, proc))
end
